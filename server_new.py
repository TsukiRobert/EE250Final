...
def handle_frame(frame: Dict[str, Any]) -> None:
    ...
    # --- which person is this? (for "new_person") ---
    person_key = None
    if persons:
        p0 = persons[0]
        if p0.get("name"):
    ...
    # --------- CREATE ONE EVENT PER FRAME WHEN THERE IS A PERSON ----------
    if event_type is not None:
        eid = next_id()

        # Save snapshot for this event (if an image was sent)
        ...
        if img_src:
            ...
            try:
                ...
                # base64 for sending back to YOLO client
    ...
    # --------- CURRENT STATE (based purely on this frame) ----------
    if flags["has_weapon"]:
    ...
    # --------- THREAT FLAGS & BLACKLIST ----------
    last_status["threat_flag"] = flags["has_weapon"]

    if flags["has_weapon"]:
        # threat image for dashboard: use snapshot for this event if it exists
        if new_event is not None:
            last_status["threat_image"] = new_event.get("snapshot_path")
        else:
            last_status["threat_image"] = None

        # threat name: known friend name or synthetic
        name = None
        for p in persons:
            if p.get("name"):
                name = p["name"]
    ...
    # --------- SNAPSHOT PAYLOAD BACK TO CLIENT ----------
    # 1) New person? send snapshot so client can store it
    ...
    # 2) Active threat? always send snapshot (if we have one)
...
def frame_result():
    ...
    threat_snapshot_b64, new_person, person_snapshot_b64, etc.
    """
